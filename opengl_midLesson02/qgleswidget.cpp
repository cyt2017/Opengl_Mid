...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    _shader.initialize();
    _rapidXML.loadScene("model/scene.xml");
    printf("_arNode.size:%d\n_map.size:%d\n",_rapidXML._arNode.size(),_rapidXML._map.size());
    fflush(NULL);
...
void QGLESWIDGET::render()
{
    ...
    CELL::matrix4   matView =   _camera.getView();
     ...
     _shader.begin();
     {
        ...
        std::vector<QMyRapidXML::Node>::iterator itr = _rapidXML._arNode.begin();
        for ( ; itr != _rapidXML._arNode.end() ; ++ itr )
        {
            QMyRapidXML::Node&           node        =   *itr;
            QMyRapidXML::Model&          model       =   _rapidXML._map[node._fileName];
            ...
            glVertexAttribPointer(_shader._positionAttr, 3,  GL_FLOAT,false,sizeof(Vertex),&model._arVertex[0].x);
            glVertexAttribPointer(_shader._uvAttr,       2,  GL_FLOAT,false,sizeof(Vertex),&model._arVertex[0].u);
            glVertexAttribPointer(_shader._colorAttr,    4,  GL_FLOAT,false,sizeof(Vertex),&model._arVertex[0].r);

            glDrawElements(GL_TRIANGLES,model._arFaces.size() * 3,GL_UNSIGNED_SHORT,&model._arFaces.front());
        }
     }
     _shader.end();
