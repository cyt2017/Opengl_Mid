...
//        glDrawElements(GL_TRIANGLES,_rapidXML._arFaces.size() * 3,GL_UNSIGNED_SHORT,&_rapidXML._arFaces.front());
    ...
    MVP     =   matProj * matMV;
    CELL::float3 vMsLightDir = CELL::normalize(CELL::float3(1, 1, 1) * matMVT);
    float fSpecularConcentration    = 0.9f;
    float fSpecularIntensity        = 0.5f;
    _shaderLight.begin();
    {
        glUniformMatrix4fv(_shaderLight.MVPMatrix,1,false,MVP.data());
        glUniform3fv(_shaderLight.LightDirection,1,&vMsLightDir.x);
        glUniform1f(_shaderLight.MaterialBias,fSpecularConcentration);
        glUniform1f(_shaderLight.MaterialScale,fSpecularIntensity / (1.0f - fSpecularConcentration));
        glUniform1i(_shaderLight.sTexture,1);

        QMyRapidXML_FaceAndVertex::Vertex vert = _rapidXML._arVertex[0];
        glVertexAttribPointer(_shaderLight.inVertex,3,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.x);
        glVertexAttribPointer(_shaderLight.inNormal,3,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.nx);
        glVertexAttribPointer(_shaderLight.inTexCoord,2,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.u);
