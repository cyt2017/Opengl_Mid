...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    _rapidXML.load("model/PerturbedUvs.xml");
    printf("_arFaces.size:%d\n_arVertexs.size:%d\n",_rapidXML._arFaces.size(),_rapidXML._arVertexs.size());
    fflush(NULL);
...
//    glBufferData(GL_ARRAY_BUFFER,_rapidXML._arVertexs.size(),&(_));
...
void QGLESWIDGET::render()
{
    ...
    CELL::matrix4 matWorld;
    matWorld.scale(0.09f,0.09f,0.09f);
     CELL::matrix4 mv  = matView*matWorld;
     CELL::matrix4 mvT = mv.inverse();
     MVP =   matProj * mv;
     QMyRapidXML_PerturbedUvs::VertexBumpMap vert = _rapidXML._arVertexs[0];
     _shaderBasicLight.begin();
     {
         glUniformMatrix4fv(_shaderBasicLight._MVP,1,false,MVP.data());
         glUniform1i(_shaderBasicLight._texture,2);
         glUniformMatrix4fv(_shaderBasicLight._matMVT,1,false,mvT.data());
         glUniform3f(_shaderBasicLight._lightDir,_camera._dir.x,_camera._dir.y,_camera._dir.z);

         glVertexAttribPointer(_shaderBasicLight._position,3,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&vert._pos.x);
         glVertexAttribPointer(_shaderBasicLight._normal,3,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&vert._normal.x);
         glVertexAttribPointer(_shaderBasicLight._uv,2,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&vert._uv.x);

         int count = _rapidXML._arFaces.size()*3;
         short *face=(short*)malloc(count*sizeof(short));
         int max = 0;
         int min = 9999;
         for(int i=0;i<_rapidXML._arFaces.size();i++)
         {
             QMyRapidXML_PerturbedUvs::Face p  = _rapidXML._arFaces[i];
             face[i*3] = p.x;
             if(max < p.x)
             {
                 max = p.x;
         ...
         glDrawElements(GL_TRIANGLES,1000,GL_UNSIGNED_SHORT,face);
     }
     _shaderBasicLight.end();
}
