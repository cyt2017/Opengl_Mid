...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    _shader.initialize();
//    _shaderBasicLight.initialize();
    _shaderDotLight.initialize();
    _rapidXML.load("model/Teapot01.sm.xml");
    printf("_arFaces.size:%d\n_arVertexs.size:%d\n",_rapidXML._arFaces.size(),_rapidXML._arVertex.size());
    fflush(NULL);
...
void QGLESWIDGET::render()
{
    ...
    CELL::matrix4   matView =   _camera.getView();
     ...
     CELL::matrix4   matProj =   CELL::perspective<float>(45.0f,float(_width)/float(_height),0.1f,10000.0f);
     ...
     _shader.begin();
     {
        ...
        glVertexAttribPointer(_shader._colorAttr,    4,  GL_FLOAT,false,sizeof(Vertex),&_dataBox._boxVertex[0].r);

        glDrawArrays(GL_TRIANGLES,0,sizeof(_dataBox._boxVertex)/sizeof(_dataBox._boxVertex[0]));


        CELL::quaternion      quat;
        CELL::matrix4   matWorld    =   CELL::makeTransform(CELL::float3(0.0f,0.0f,0.0f),CELL::float3(1.0f,1.0f,1.0f),quat);

        MVP =   matProj * matView * matWorld;
        glUniformMatrix4fv(_shader._MVP,1,false,MVP.data());
        glUniform1i(_shader._texture,1);

        glVertexAttribPointer(_shader._positionAttr, 3,  GL_FLOAT,false,sizeof(Vertex),&_rapidXML._arVertex[0].x);
        glVertexAttribPointer(_shader._uvAttr,       2,  GL_FLOAT,false,sizeof(Vertex),&_rapidXML._arVertex[0].u);
        glVertexAttribPointer(_shader._colorAttr,    4,  GL_FLOAT,false,sizeof(Vertex),&_rapidXML._arVertex[0].r);

        glDrawElements(GL_TRIANGLES,_rapidXML._arFaces.size() * 3,GL_UNSIGNED_SHORT,&_rapidXML._arFaces.front());
...
//    angle += 0.1;
//    CELL::matrix4 matRotY;
//    matRotY.rotateY(angle);

//    CELL::matrix4 matWorld  =   CELL::makeTransform(CELL::float3(0.0f,0.0f,0.0f),CELL::float3(1.0f,1.0f,1.0f),quat);
//    CELL::matrix4 matMV     =   matView * matWorld * matRotY;
//    CELL::matrix4 matMVT    =   matMV.inverse();

//    MVP     =   matProj * matMV;
    _shaderDotLight.begin();
    {


//        glUniformMatrix4fv(_shaderDotLight._MVP,1,false,MVP.data());
//        glUniformMatrix4fv(_shaderDotLight._matMV,1,false,matMV.data());
//        glUniformMatrix4fv(_shaderDotLight._matMVT,1,false,matMVT.data());
//        glUniform1i(_shaderDotLight._texture,1);

//        CELL::float3  lightPos(50,50,-100);
//        CELL::float3  lightDir(_camera._dir.x,_camera._dir.y,_camera._dir.z);
//        CELL::float3  ightColor(1.0f,1.0f,1.0f);
//        glUniform3f(_shaderDotLight._lightPos,lightPos.x,lightPos.y,lightPos.z);
//        glUniform3f(_shaderDotLight._lightDir,lightDir.x,lightDir.y,lightDir.z);
//        glUniform3f(_shaderDotLight._lightColor,ightColor.x,ightColor.y,ightColor.z);

//        QMyRapidXML_FaceAndVertex::Vertex vert = _rapidXML._arVertex[0];
//        glVertexAttribPointer(_shaderDotLight._position,3,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.x);
//        glVertexAttribPointer(_shaderDotLight._normal,3,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.nx);
//        glVertexAttribPointer(_shaderDotLight._uv,2,GL_FLOAT,false,sizeof(QMyRapidXML_FaceAndVertex::Vertex),&vert.u);

//        glDrawElements(GL_TRIANGLES,600,GL_UNSIGNED_SHORT,&(_rapidXML._arFaces.front()));
    }
    _shaderDotLight.end();
}
