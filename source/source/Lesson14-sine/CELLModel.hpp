...
namespace   CELL
{
    ...
    public:
        ...
        *   绘制函数
        */
        virtual void    onRender(Program_SINE& shader,CELL3RDCamera& camera)
        {
            ...
            matrix4 matModel    =   matTrans * matScale;
            ...
            _device.setUniform1f(shader.myAnim,angle);
...
#else
            std::vector<VertexBumpMap> arVertexs    =   _arVertexs; 
            for (size_t i  = 0 ;i < arVertexs.size() ; ++ i )
            {
                float ripple = 5.0*cos(0.2*arVertexs[i]._pos.y + (DEG2RAD(5.0*angle*360.0)));
                arVertexs[i]._pos.z +=  ripple;
            }
            VertexBumpMap*  vert    =   &arVertexs.front();
#endif
