...
void QGLESWIDGET::render()
{
    ...
    CELL::matrix4   matView =   _camera.getView();
     ...
     _shaderFog.begin();
     {
        ...
        CELL::matrix4 mv  =   _camera._matView * matModel ;
        CELL::matrix4 mvI =   mv.inverse();

        CELL::float4  vEyePosModel;
        vEyePosModel = mvI * CELL::float4(0, 0, 0, 1);
        ...
        glUniform1f(_shaderFog.FogEnd,          fFogEnd);
        glUniform1f(_shaderFog.FogDensity,      fFogDensity);
        glUniform3fv(_shaderFog.FogColor, 1,    afFogColor);

        glUniform3fv(_shaderFog.EyePositon, 1,     (float*)&_camera._eye);

        glUniformMatrix4fv(_shaderFog.MVPMatrix,1,false,MVP.data());

        glUniformMatrix4fv(_shaderFog.ModelViewMatrix,1,false,mv.data());

        QMyRapidXML_FaceAndVertex::Vertex vert = _rapidXML._arVertex[0];
        glVertexAttribPointer(_shaderFog.inVertex,   3,  GL_FLOAT,   GL_FALSE, sizeof(Vertex), &vert.x);
        glVertexAttribPointer(_shaderFog.inNormal,   3,  GL_FLOAT,   GL_FALSE, sizeof(Vertex), &vert.nx);
        glVertexAttribPointer(_shaderFog.inTexCoord, 2,  GL_FLOAT,   GL_FALSE, sizeof(Vertex), &vert.u);
