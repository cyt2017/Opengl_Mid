...
void QGLESWIDGET::render()
{
    ...
    CELL::matrix4   matView =   _camera.getView();
     ...
     _shaderCar.begin();
     {
        ...
        matTrans.translate(0,0,0);
        CELL::matrix4 matScale(1);
        matScale.scale(1.0f,1.0f,1.0f);

        CELL::matrix4 matModel    =   matTrans * matRot * matScale;
        angle   +=  0.001f;
        // transform directional light from world space to model space
        CELL::float4  vMsLightDir     =   CELL::float4(1, 2, 1, 0) * matModel;
        CELL::float3  vMsLightDirS    =   CELL::normalize(CELL::float3(vMsLightDir.x,vMsLightDir.y,vMsLightDir.z));


        CELL::float4  vMsLightPos = CELL::float4(50, 20, 40, 1);
        vMsLightPos =   vMsLightPos * matModel;
        CELL::matrix4 MVP =   _camera._matProj * _camera._matView * matModel ;

        glUniform1i(_shaderCar.sTexture,1);

        glUniformMatrix4fv(_shaderCar.MVPMatrix,1,false,MVP.data());
        glUniform3fv(_shaderCar.EyePosition,1,&vMsLightPos.x);
        glUniform3fv(_shaderCar.LightDirection,1,&vMsLightDirS.x);
#if 1
        glUniform1f(_shaderCar.myAnim,angle);
        glVertexAttribPointer(_shaderCar.inVertex,3,GL_FLOAT,false,sizeof(Vertex),&(_rapidXML._arVertex[0].x));
        glVertexAttribPointer(_shaderCar.inNormal,3,GL_FLOAT,false,sizeof(Vertex),&(_rapidXML._arVertex[0].nx));

        glDrawElements(GL_TRIANGLES,_rapidXML._arFaces.size()*3,GL_UNSIGNED_SHORT,&_rapidXML._arFaces.front());
#else
        std::vector<QMyRapidXML_FaceAndVertex::Vertex> vert = _rapidXML._arVertex;
        for(int i=0;i<vert.size();i++)
        {
            float ripple = 2.0*cos(0.2*vert[i].y + (DEG2RAD(8.0*angle*360.0)));
            vert[i].z  +=  ripple;
        }
        glVertexAttribPointer(_shaderCar.inVertex,3,GL_FLOAT,false,sizeof(Vertex),&(vert[0].x));
        glVertexAttribPointer(_shaderCar.inNormal,3,GL_FLOAT,false,sizeof(Vertex),&(vert[0].nx));
