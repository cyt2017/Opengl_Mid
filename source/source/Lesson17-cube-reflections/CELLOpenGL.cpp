...
namespace   CELL
{
    ...
    */
    ...
    {
        bindTexture(GL_TEXTURE_2D,textureId ? textureId->_texture : 0,stage);
    }


    void CELLOpenGL::bindTexture( unsigned target,unsigned textureId,unsigned stage )
    {
        ...
        if (textureId)
        {
            if (sTexture == textureId && sStage == stage)
            {
                return;
            }
            else
    ...
    *   创建一个立体文理
    */
    CELL::TextureCubeId CELLOpenGL::createCubeTextureFromFile( const char* files[] )
    {
        TextureCubeId   texId;
        glGenTextures(1, &texId._texture);
        glBindTexture(GL_TEXTURE_CUBE_MAP, texId._texture);
        ...
        {
            unsigned    length  =   0;
            char*       buffer  =   readFile(files[i],length);
            if (buffer == 0 || length == 0)
            {
                return  texId;
            }
            CELLImageReader reader((char*)buffer,(int)length);
            if(!reader.parse())
            {
                delete [] buffer;
                return  texId;
    ...
    void CELLOpenGL::bindTextureCube( TextureCubeId* tex,int stage /*= 0*/ )
    {
        bindTexture(GL_TEXTURE_CUBE_MAP,tex ? tex->_texture : 0,stage);
    }

    void CELLOpenGL::destroyTextureCube( TextureCubeId& id )
    {
        glDeleteTextures(1,&id._texture);
    }
