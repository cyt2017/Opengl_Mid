...
namespace   CELL
{
    ...
    public:
        ...
        *   绘制函数
        */
        virtual void    onRender(Program_TNL& shader,CELL3RDCamera& camera)
        {
            ...
            matrix4 mvI =   mv._inverse();

            float3 vMsLightDir = normalize(float3(1, 1, 1) * mvI);
            ...
            float fSpecularConcentration    = 0.9f;	    // a value from 0 to 1 (wider, concetrated)
            float fSpecularIntensity        = 0.5f;	    // a value from 0 to 1

            // Specular bias
            _device.setUniform1f(shader.MaterialBias, fSpecularConcentration);
            // Specular intensity scale
            _device.setUniform1f(shader.MaterialScale, fSpecularIntensity / (1.0f - fSpecularConcentration));
