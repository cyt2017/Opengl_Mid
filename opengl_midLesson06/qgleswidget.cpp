...
//            _device.setUniform3fv(shader.LightPosModel,1,&vMsLightPos.x);


            QMyRapidXML_PerturbedUvs::VertexBumpMap  vert    =   _rapidXMLP._arVertexs[0];
...
//            _device.drawElements(GL_TRIANGLES,_arFaces.size() * 3,GL_UNSIGNED_SHORT,&_arFaces.front());
         ...
         glUniform1i(_shaderNor._texNor,1);
         glUniform1i(_shaderNor._texNor1,2);
         glUniformMatrix4fv(_shaderNor._MVP,1,false,MVP.data());
         glUniform3fv(_shaderNor._lightPos,1,&vMsLightPos.x);

         glVertexAttribPointer(_shaderNor._position,3,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&(vert._pos.x));
         glVertexAttribPointer(_shaderNor._tangent,3,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&(vert._tangent.x));
         glVertexAttribPointer(_shaderNor._normal,3,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&(vert._normal.x));
         glVertexAttribPointer(_shaderNor._uv,2,GL_FLOAT,false,sizeof(QMyRapidXML_PerturbedUvs::VertexBumpMap),&(vert._uv.x));

         glDrawElements(GL_TRIANGLES,300,GL_UNSIGNED_SHORT,&_rapidXMLP._arFaces[0]);
     }
     _shaderNor.end();
}
